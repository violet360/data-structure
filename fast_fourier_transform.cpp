...
int main()
{
  ...
  ios_base::sync_with_stdio(false);cin.tie(0);cout.precision(20);srand(time(0));
	...
	{
		int m, n, inp;
		cin>>n>>m; //enter length of each polynomial
		vector<int> f1,f2;  //f1(x) = a0 + a1*x + a2*(x^2) + ..... a(m-1)(x^m-1) ; f2(x) = b0 + b1*x + b2*(x^2) + ..... b(n-1)(x^n-1)
		
		for(int i = 0;i<n;i++)
		{
			cin>>inp;
			f1.push_back(inp); // a0, a1, a2, a3, ...
		}
		for(int i = 0;i<m;i++)
		{
			cin>>inp;
			f2.push_back(inp); //b0, b1, b2, b3, ...
		}
		multiply(f1,f2);
	}
	return 0;
